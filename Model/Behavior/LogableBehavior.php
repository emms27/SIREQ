...
if (!defined('CLASS_USER')) {define('CLASS_USER', 'User');}
 ...
 * - "change" 	[string] : depending on setting either : [name (alek) => (Alek), age (28) => (29)] or [name, age]
 *
...
class LogableBehavior extends ModelBehavior {
	...
	public function setUserIp(Model $Model, $userIP = null) {
		if ($userIP === null) $userIP = CakeRequest::clientIp();
			//App::uses();
		$this->userIP = $userIP;
	}
	...
	public function afterSave(Model $Model, $created) {
		...
		if ($this->Log->hasField('description')) {
			...
			if ($created) $logData['Log']['description'] .= __('added');
			else $logData['Log']['description'] .= __('updated');
		}
		if ($this->Log->hasField('action')) {
			if ($created) $logData['Log']['action'] = 'add';
			else $logData['Log']['action'] = 'edit';			
		}
	...
	public function _saveLog(Model $Model, $logData, $title = null) {
		if ($title !== null) $logData['Log']['title'] = $title;
		...
		if ($this->Log->hasField($this->settings[$Model->alias]['classField'])) 
	      ...
	      if ($this->Log->hasField($this->settings[$Model->alias]['foreignKey']) && !isset($logData['Log'][$this->settings[$Model->alias]['foreignKey']])) {
	        if ($Model->id) $logData['Log'][$this->settings[$Model->alias]['foreignKey']] = $Model->id;
	        elseif ($Model->insertId) $logData['Log'][$this->settings[$Model->alias]['foreignKey']] = $Model->insertId;
	      }
		...
		//if ($this->Log->hasField('ip') && $this->userIP) $logData['Log']['ip'] = $this->userIP;
		if ($this->Log->hasField('ip')) $logData['Log']['ip'] = CakeRequest::clientIp();
